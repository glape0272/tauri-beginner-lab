...
- 完全初心者: まず仮データを画面に出すところまで進めます。
- フロントエンド経験者: TypeScriptでの描画部分は自分の書き方に置き換えても構いません。
- PHP/Laravel経験者: 最初はDB保存ではなく、固定配列を一覧表示する練習として進めます。

## ここで作るもの

「学習ログ」の入口を作ります。

この章では、まだファイル保存はしません。まずは画面上に学習メモの一覧を出し、後から保存機能を足せる形にします。
...
おすすめは「Rust側で仮のログ一覧を返す」です。ファイル保存に進む前に、Rustの`struct`配列をTypeScriptへ返す練習になります。
